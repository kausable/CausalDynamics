...
def solve_system(
    ...
    num_timesteps : int
        Number of timesteps to simulate
    num_nodes : int
        Number of nodes (separate instances) to simulate
    system_name : str
    ...
    -------
    torch.Tensor
        Tensor of shape (num_timesteps, num_nodes, num_dimensions) containing
    ...
    try:
        ...
        # (num_timesteps, num_nodes, num_dimensions) -> (num_nodes, num_timesteps, num_dimensions)
...
def solve_random_systems(
    ...
    num_nodes : int
        Number of nodes/systems to simulate
    make_trajectory_kwargs : dict
        kwargs passed on the dysys.make_trajectory
    max_retry : int
    ...
    -------
    torch.Tensor
        Tensor of shape (num_timesteps, num_nodes, 3) containing
        the solution trajectories for all nodes
    """
    systems_dim3 = get_3d_chaotic_system_names()
    # Randomly select N different attractors from systems_dim3
    selected_systems = random.sample(systems_dim3, min(num_nodes, len(systems_dim3)))
...
def get_adjacency_matrix_from_jac(dyn_sys):
    """
    ...
    ----------
    dyn_sys : DynSys
        The dynamical system class instance with a _jac method.
        If _jac is None or not present, the function returns a placeholder value.

    Returns
    -------
    numpy.ndarray
        Binary adjacency matrix where 1 indicates a non-zero entry in the Jacobian.
        Returns a scalar array with value 1 if the dynamical system doesn't have a _jac method
        or if the system is not a standard 3D system.
    ...
    For systems without a Jacobian method or non-standard dimensionality, a placeholder
    value (scalar array with value 1) is returned instead of None to maintain consistency
    in return types.
    """
