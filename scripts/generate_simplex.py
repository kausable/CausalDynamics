...
def generate(*, data_dir: str, noise: float, confounder: bool, system_name: str):
    """
    Generate all data, ranging from uncoupled (simplex) models.
    ...
    -------
    None
        Results are saved to disk at data_dir/output/simplex/<experiment_id>/<system_name>.nc as NetCDF files

    Notes
    -----
    Example usage:
        python scripts/generate_simplex.py --data_dir data --noise 0.5 --confounder --system_name Lorenz84
    ...
    # Directory setup
    experiment_dir = (
        Path(data_dir) / "simplex" / f"noise={noise:.2f}_confounder={confounder}"
    )
